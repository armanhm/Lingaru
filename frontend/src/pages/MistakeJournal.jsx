...
export default function MistakeJournal() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold text-surface-900 dark:text-surface-100 mb-6">Mistake Journal</h1>

      <div className="flex flex-wrap gap-3 mb-6">
        <select
          value={filter.type}
          onChange={(e) => handleFilterChange("type", e.target.value)}
          className="px-3 py-2 border border-surface-300 dark:border-surface-600 rounded-lg text-sm dark:bg-surface-700 dark:text-surface-100"
        >
      ...
      {loading ? (
        ...
        <p className="text-surface-500 dark:text-surface-400 text-center py-12">No mistakes found. Keep practicing!</p>
      ) : (
        <div className="space-y-3">
          {mistakes.map((m) => (
            <div
              key={m.id}
              className={`bg-white dark:bg-surface-800 rounded-xl border dark:border-surface-700 p-4 flex items-start gap-3 ${
            ...
            >
              ...
              <div className="flex-1">
                {m.question_prompt && (
                  <p className="text-sm text-surface-500 dark:text-surface-400 mb-1">{m.question_prompt}</p>
                )}
              ...
              <span className="text-xs font-medium px-2 py-1 bg-surface-100 dark:bg-surface-700 rounded-full text-surface-600 dark:text-surface-400">
